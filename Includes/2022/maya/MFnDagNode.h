...
public:
	...
	OPENMAYA_DEPRECATED(2016, "Use MFnDagNode::objectColorIndex() and MFnDagNode::objectColorRGB() instead")
	int				objectColor( MStatus * ReturnStatus = NULL ) const;

	MStatus			setObjectColor( int color );

	OPENMAYA_DEPRECATED(2016, "Use MFnDagNode::objectColorType() instead.")
	bool			usingObjectColor( MStatus * ReturnStatus = NULL ) const;

	OPENMAYA_DEPRECATED(2016, "Use MFnDagNode::setObjectColorType() instead")
	...
	OPENMAYA_DEPRECATED(<2016, "The model returned is an unknown type to the API and now the method always returns MObject::kNullObj.")
