...
public:
	...
	OPENMAYA_DEPRECATED(2022.4, "MPxTransform should not manage built-in attribute Translation")
	MStatus _dirtyTranslation(const MVector &v)				{ return dirtyTranslation(v); }
	OPENMAYA_DEPRECATED(2022.4, "MPxTransform should not manage built-in attribute Rotation")
	MStatus _dirtyRotation(const MEulerRotation &e)			{ return dirtyRotation(e); }
	OPENMAYA_DEPRECATED(2022.4, "MPxTransform should not manage built-in attribute Scale")
	MStatus _dirtyScale(const MVector &v)					{ return dirtyScale(v); }
	OPENMAYA_DEPRECATED(2022.4, "MPxTransform should not manage built-in attribute Shear")
	MStatus _dirtyShear(const MVector &v)					{ return dirtyShear(v); }
	OPENMAYA_DEPRECATED(2022.4, "MPxTransform should not manage built-in attribute RotateOrientation")
	MStatus _dirtyRotateOrientation(const MEulerRotation &e){ return dirtyRotateOrientation(e); }
	OPENMAYA_DEPRECATED(2022.4, "MPxTransform should not manage built-in attribute ScalePivot")
	MStatus _dirtyScalePivot(const MPoint &p)				{ return dirtyScalePivot(p); }
	OPENMAYA_DEPRECATED(2022.4, "MPxTransform should not manage built-in attribute RotatePivot")
	MStatus _dirtyRotatePivot(const MPoint &p)				{ return dirtyRotatePivot(p); }
	OPENMAYA_DEPRECATED(2022.4, "MPxTransform should not manage built-in attribute ScalePivotTranslation")
	MStatus _dirtyScalePivotTranslation(const MVector &v)	{ return dirtyScalePivotTranslation(v); }
	OPENMAYA_DEPRECATED(2022.4, "MPxTransform should not manage built-in attribute RotatePivotTranslation")
...
// ****************************************************************************
// CLASS DECLARATION (MPxTransformPreRotation)

//! \ingroup OpenMaya MPx
//! \brief Base class for user defined transforms with pre-rotation.
/*!
This is a temporary class that allows a user defined transform to supply
...
This functionality will be added directly to MPxTransform in a future
release and this class will be removed at that point.
*/
class OPENMAYA_EXPORT MPxTransformPreRotation : public MPxTransform
{
public:
