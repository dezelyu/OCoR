...
#define DeclareCheckpointed()														\
	public:																			\
		Checkpoint classCheckpoint() override;										\
		bool       classChangedSinceCheckpoint( const Checkpoint& check ) override;	\
		Checkpoint classChanged() override;											\
		DeclareCheckpointedStatics()
...
namespace adsk {
	...
	using Checkpoint = uint64_t; //!< Make it big enough to handle tons and tons of edits
